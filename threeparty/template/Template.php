...
class Template
{
     ...
     * @access public
     */
    public function set_custom_template($template_path, $cache_path)
    {
        $this->root = $template_path;
        $this->cachepath = $cache_path . '/tpl_';
        return true;
     ...
     * @access public
     */
    public function set_language_var(&$lang)
    {
        $this->lang = &$lang;
    }
    ...
    private function _tpl_load(&$handle)
    {
        ...
        // echo "str:".strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE'])."<br>";
        // echo $filename.": ".filemtime($filename);
        $lastModify = 0;
        if (isset($_SERVER)) {
            if (@isset($_SERVER['HTTP_IF_MODIFIED_SINCE'])) {
                $lastModify = $_SERVER['HTTP_IF_MODIFIED_SINCE'];
            }
        }
        if (is_file($filename)) {
            if (strtotime($lastModify) > filemtime($filename)) {
                $recompile = true;
                // echo "<br>ENTRA";
            } elseif (@filemtime($filename) < filemtime($this->files[$handle])) {
                $recompile = true;
        ...
        if (! $recompile)
            ...
            // echo "<br>Recarga: ".$this->files[$handle];
    ...
    /**
     * assign_blocks
     * Función auxiliar para hacer que la asignación de los bloques sea a través de la vista Page en vez desde esta clase de plantillas
     *
     * @access public
     */
    public function assign_blocks($blocks)
    {
        $this->_tpldata = $blocks;
    }
...
[^\s(),]+)/x', $tag_args, $match);
     ...
     * (possibly nested) block namespace.
     * This is a string of the form:
